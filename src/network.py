...
from utils.ops import Initializer
...
class GNet(nn.Module):
    def __init__(self, in_dim, n_classes, args):
        ...
        GConv = getattr(layers, args.g_conv)
        GPool = getattr(layers, args.g_pool)
        self.s_gcn = GConv(in_dim, args.l_dim)
        self.network_drop = nn.Dropout(p=args.drop_n)
        self.convs = nn.ModuleList([
            GConv(args.l_dim, args.l_dim)
            for _ in range(args.l_num)
        ])
        if args.g_pool == 'GPool':
            self.pools = nn.ModuleList([
                GPool(args.ks[i], args.l_dim)
                for i in range(args.l_num)
            ])
        else:
            self.pools = nn.ModuleList([
                GPool(args.clusters[i], args.l_dim)
                for i in range(args.l_num)
            ])
